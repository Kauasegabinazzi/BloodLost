...
class BloodLostGame:
    ...
    def check_first_time(self):
        """Verifica se é a primeira vez que o jogo está sendo executado"""
        try:
            if os.path.exists("language_settings.json"):
                with open("language_settings.json", "r") as f:
                    data = json.load(f)
    ...
    def draw_colorblind_setup(self):
        ...
        subtitle_text = (
            ...
            else "Select color mode for best experience:"
        )
        subtitle_surf = self.resource_manager.fonts["small"].render(
            subtitle_text, False, WHITE
        )
        ...
        for i, mode in enumerate(COLORBLIND_ORDER):
            ...
            if i == self.selected_colorblind:
                descriptions = {
                    "normal": {
                        "pt": "Visao padrao sem ajustes",
                        "en": "Standard vision without adjustments"
                    },
                    "protanomalia": {
                        "pt": "Dificuldade em ver tons de vermelho",
                        "en": "Difficulty seeing red tones"
                    },
                    "deuteranomalia": {
                        "pt": "Dificuldade em ver tons de verde",
                        "en": "Difficulty seeing green tones"
                    },
                    "tritanomalia": {
                        "pt": "Dificuldade em ver tons de azul",
                        "en": "Difficulty seeing blue tones"
                    }
                }
                
                desc_text = descriptions[mode][self.language_manager.current_language]
                desc_surf = self.resource_manager.fonts["small"].render(
                    desc_text, False, LIGHT_GRAY
                )
        ...
        instructions_text = (
            ...
            if self.language_manager.current_language == "pt"
            else "Use ARROWS to navigate, ENTER to confirm"
        )
        instructions_surf = self.resource_manager.fonts["small"].render(
            instructions_text, False, DARK_GRAY
        )
        ...
        preview_text = (
            ...
            if self.language_manager.current_language == "pt"
            else "Current mode: {}".format(COLORBLIND_TYPES[COLORBLIND_ORDER[self.selected_colorblind]]["en"])
        )
        preview_surf = self.resource_manager.fonts["small"].render(
            preview_text, False, GOLD
        )
        preview_rect = preview_surf.get_rect(center=(400, 560))
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            # Adicionar handler para colorblind_setup
            if self.game_state == "colorblind_setup":
                self.handle_colorblind_setup_events(event)
    ...
    def render(self):
        ...
        elif self.game_state == "colorblind_setup":
            self.draw_colorblind_setup()
        elif self.game_state == "menu":
            self.draw_menu()
        elif self.game_state == "instructions":
